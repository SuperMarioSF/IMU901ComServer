...
func DecodeStart(port serial.Port, gyroRange DeviceGyroRange) error {
	...
	case QUATERNION:
		...
		doUpdateMessage(quaternionData)
		break
	case GYRO:
...
func doUpdateMessage(data DecodedData) {
	str, err := data.ToJson()
	if err != nil {
		return // just ignore for now
	}
	ws_server.ProcessUpdateEvent(str)
}
